...
export function ExpandingClickablePhoto({ data, setstate, centercrop }) {
  return (
    <div className="w-full h-full relative">
      ...
      <div className="tutorialtext xl:text-2xl z-10 lg:text-xl md:text-lg  absolute bottom-0 left-0 pointer-events-none p-4 md:pb-16 xss:pb-14">
        {data.title}
      </div>
      <div
      ...
      >
        <div 
          className="spyagencyGradient polygon absolute bottom-0 "
        >
          <div className="bg-white xss:py-1 py-2 xss:text-md md:text-lg lg:text-xl xl:text-2xl 
          xss:px-6 xss:pr-2 md:pl-4 md:pr-8 xl:pr-10">Open Gallery</div>
        </div>
      </div>
    </div>
  );
